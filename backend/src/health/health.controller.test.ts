...
(global as unknown as { fetch: typeof mockedFetch }).fetch = mockedFetch;

function buildClient(query: jest.Mock): { query: jest.Mock; release: jest.Mock } {
  return {
    query,
    release: jest.fn(),
  };
...
describe("HealthController", () => {
  ...
  it("calls the configured Soroban RPC URL with a getHealth JSON-RPC request", async () => {
    ...
    expect(mockedFetch).toHaveBeenCalledWith(
      "https://soroban-test.example.com",
